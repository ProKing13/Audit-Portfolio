...
- 🟡 **M-01.** [Incorrect Flag Check for Deposit Pause in `ModuleState::LVDepositNotPaused` Modifier](#m-01)

---

<a id="m-01"></a>
## 🟡 **M-01 - Incorrect Flag Check for Deposit Pause in `ModuleState::LVDepositNotPaused` Modifier**
...
Update the `ModuleState::LVDepositNotPaused` modifier to check the correct flag, such as `isDepositPaused`, rather than `isWithdrawalPaused`.

**Suggested fix:**

```solidity
modifier LVDepositNotPaused(Id id) {
    // Change the condition to check isDepositPaused instead of isWithdrawalPaused
+   if (states[id].vault.config.isDepositPaused) {
-   if (states[id].vault.config.isWithdrawalPaused) {
        revert LVDepositNotPaused();
    }
    _;
}
```
